...
public class BeanQuery<T> extends AbstractQuery {
	...
	public static <R extends BeanMain<?, ?>> BeanQuery<R> SELECT(Class<R> beanClass) {
		BeanQuery<R> query = new BeanQuery<>();
		query.beanClass = beanClass;
		query.sql = new SQL().SELECT(beanClass).FROM(beanClass);
		return query;
	}
	
	public static BeanQuery<?> SELECT(IEnumFld... flds) {
		BeanQuery<?> query = new BeanQuery<>();
		query.sql = new SQL().SELECT(flds);
		return query;
	}
	
	public static <R extends BeanMain<?, ?>> BeanQuery<R> UPDATE(Class<R> beanClass) {
		BeanQuery<R> query = new BeanQuery<>();
		query.beanClass = beanClass;
		query.sql = new SQL().UPDATE(beanClass);
		return query;
	}
	
	public static <R extends BeanMain<?, ?>> BeanQuery<R> DELETE(Class<R> beanClass) {
		BeanQuery<R> query = new BeanQuery<>();
		query.beanClass = beanClass;
		query.sql = new SQL().DELETE_FROM(beanClass);
		return query;
	}
